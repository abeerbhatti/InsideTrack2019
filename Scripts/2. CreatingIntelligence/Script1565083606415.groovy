...
int TotalRows = findTestData('Resource').getRowNumbers()
for (GlobalVariable.i=1; GlobalVariable.i<=TotalRows; GlobalVariable.i++){
	WebUI.verifyElementVisible(findTestObject('ResourcePage/addResource'), FailureHandling.CONTINUE_ON_FAILURE)
	System.out.println("Number" + GlobalVariable.i + "of " + TotalRows + "Resource")
	WebUI.callTestCase(findTestCase('createResourceFile'), [:], FailureHandling.STOP_ON_FAILURE)
	WebUI.delay(2)
}